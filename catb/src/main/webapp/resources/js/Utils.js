...
// Xóa trắng nội dung 2 textbox
function clear2TextBox(txtId1, txtId2) 
{	
	var textbox1 = document.getElementById(txtId1);
	textbox1.value = "";
	var textbox2 = document.getElementById(txtId2);
	textbox2.value = "";
}
// Xóa trắng nội dung textbox
...
////////Edit by phucnt
// phucntbk@gmail.com
// txt.Attributes.Add("onblur", "checkdate('" + txt.ClientID + "')");
////////
function checkdate(txtId){
var checkstr = "0123456789";
var DateField = document.getElementById(txtId);
var Datevalue = "";
var DateTemp = "";
var seperator = "/";
var day;
...
var i;
   ...
   /* Xóa các ký tự không nằm trong khoảng 0..9 */
   for (i = 0; i < DateValue.length; i++) {
	  ...
	  if (DateTemp == "")  
	  {
	  alert("Ký tự nhập vào phải là kiểu số!");
	  DateField.focus();
	  break;
	  }
   ...
   DateValue = DateTemp;
   /* Chuyển về 8 ký tự*/
   /* Nếu năm có 2 ký tự thì thêm 20 vào trước / chuyển về dạng 20xx */
   if (DateValue.length == 6) {
      DateValue = DateValue.substr(0,4) + '20' + DateValue.substr(4,2); }
   if (DateValue.length != 8) {
      err = 19;}
   /* năm sai nếu năm = 0000 */
   ...
   /* nếu là ký tự 00 thì xóa đi */
   if ((day == 0) && (month == 0) && (year == 00)) {
      err = 0; day = ""; month = ""; year = ""; seperator = "";
   }
   /* nếu không có lỗi thì ghi ra theo định dạng dd/mm/yyyy(vd: 13.12.2001) */
   if (err == 0) {
...
* On 09/05/2k8
*/
function openModalDialog(dialogUrl)
{
var dialogFeatures = 'dialogWidth:620px; dialogHeight:200px; center:yes; edge:sunken; help:no; resizable:no; scroll:yes; status:no;' 
dialogResults = window.showModalDialog(dialogUrl, null, dialogFeatures);
}

/*	
* Ẩn hiện thẻ div trong các form cần dùng			
* 06/02/2006
* Tạo mới
*/
function ShowOrHideDivPanel(theDiv)
{
...
function checkOneExtend(selObj, selChk, dgrName)
{
     var items=document.Form1 ;
     for(var i=0;i<items.length;i++)
     {
          if(items.elements[i].type=="radio")
          {
               if(items.elements[i].id.indexOf(dgrName)>-1)
                    items.elements[i].checked=false;
          }          
     }     
     document.getElementById(selObj).checked=true;
     re = new RegExp(':' + selChk + '$')      

}

//////////////////////////////////////////////////////////////////////////////
function checkOne(selObj, dgrName)
{
     var items=document.Form1 ;
     for(var i=0;i<items.length;i++)
     {
...
function clickBienMat(txtObj)
{
	document.getElementById(txtObj).value = '';
	document.getElementById(txtObj).className = 'textbox';
}

function clickBienMat_Date(txtObj)
{	
	document.getElementById(txtObj).value = '';
	document.getElementById(txtObj).className = 'textboxdate';
}

// Kiểm tra textbox có phải là kiểu số ko
	function txtValid_Value(txtId)
{
	//var checkstr = "0123456789";
    var IntField = document.getElementById(txtId);
    oneDecimal = false;
	inputStr = IntField.value;
	for (var i = 0; i < inputStr.length; i++) {
		...
		alert("Dữ liệu nhập vào phải là kiểu số");
		IntField.focus();
		IntField.value = "";
		return false;
		}
...
/*
Hàm thực hiện kiểm tra giá trị nhập vào có phải là số hay không
*/


function txtValid(strInputArray, idValidator)
{
	var sInputArray = strInputArray.split("|");
	var icount = 0;
	var txt="";
	var obj, obj1;
	var bReturn = true;
	for(icount=0; icount<sInputArray.length; icount++)
	{
		obj1=document.getElementById(sInputArray[icount]);
		txt = obj1.value;
		if(txt=="")
		{
			obj = document.getElementById(idValidator);
			obj.value="Vui lòng nhập dữ liệu vào!";
			event.cancelBubble();
			bReturn = false;
			break;
		}
...
function txtValidator(strInputArray)
{
	var sInputArray = strInputArray.split("|");
	var icount = 0;
	var txt="";
