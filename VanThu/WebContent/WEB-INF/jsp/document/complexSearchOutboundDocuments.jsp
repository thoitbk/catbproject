...
function report() {
	...
	var url = '/document/complexReportOutbound.html?ssign=' + sign + '&stype=' + type + '&sconfidentialLevel=' + confidentialLevel + '&surgentLevel=' + urgentLevel + 
			'&sreceiver=' + receiver + '&ssigner=' + signer + '&sfromPublishDate=' + fromPublishDate + '&sfromSendDate=' + fromSendDate +
...
<div class="box960">
	...
	<div class="content_960">
		<form action="/document/complexSearchOutboundDocuments.html" method="GET">
...
<div class="khung_content_960" id="gridCongViec">
	...
	<div class="content-grid">
		<table class="gridView" cellspacing="1" style="display: block; clear: both; width: 100%;">
			...
			<c:forEach items="${documents}" var="document" varStatus="s">
				...
				<tr class="${c}">
					...
					<td>
						<a href="javascript:void(0);" onclick="window.open('/document/outboundDocumentDetails.html?d=${document.id}', '_blank', 'toolbar=yes, scrollbars=yes, resizable=yes, top=100, left=100, width=1100, height=600');">
							${document.sign}
						</a>
					</td>
					<td>${document.abs}</td>
					...
					<td>
						<c:forEach items="${document.documentFiles}" var="file">
							<a href="/document/downloadOutboundDocument.html?f=${file.id}">${file.name}</a><br />
						</c:forEach>
					</td>
				</tr>
			</c:forEach>
		...
		<div class="bottom-pager">
			<div class="left">
				<div class="pages">
					<t:paging pageInfo="${pageInfo}" link="/document/complexSearchOutboundDocuments.html" params="${params}"/>
				</div>
			</div>
			<div class="right">
