...
$(function() {
	...
	<c:forEach items="${departments}" var="department" varStatus="s">
		__senders[${s.count - 1}] = ${department.key};
	</c:forEach>
	...
	$("#updateComingDocumentViewModel").submit(function(event) {
		...
		$.post($(this).attr("action"), { 
		  ...
		  .done(function( data ) {
			  ...
			  if (data.code == 0) {
				  ...
				  window.location.href="/comingDocument/updateDocument/${document.id}.html";
			  }
		  });
	});
});
...
<div class="box960">
	...
	<div class="content_960">
		<form action="/comingDocument/updateDocument.html" method="get" id="updateComingDocumentViewModel">
			<div class="errorblock" id="error"></div>
			<input type="hidden" id="documentId" name="documentId" value="${document.id}" />
			<input type="hidden" id="documentSign" name="documentSign" value="${document.sign}" />
				...
				<tr>
					...
					<td>
						<select multiple="multiple" size="8" style="width:40%;" id="senders_" name="senders_" class="senders_">
							<c:forEach items="${nDepartments}" var="nDepartment">
								<option value="${nDepartment.key}">${nDepartment.value}</option>
							</c:forEach>
						</select>
					...
					<td>
						<div id="container" style="float: left;">
							<div id="upload-file-container" class="fade well">
								<input id="fileupload" type="file" name="files[]" data-url="/comingDocument/uploadDocument.html" multiple>
							</div>
				...
				<tr>
					...
					<td>
						<div id="uploaded-files">
							<c:if test="${comingFiles != null && !empty comingFiles}">
								<c:forEach items="${comingFiles}" var="file">
									<div id="${file.id}">
...
function delDocument(id) {
	$.getJSON( "/comingDocument/deleteDocument.html?id=" + id, function( json ) {
		if (json.code == 1) {
			$("#" + id).remove();
		}
	});
